...
const { expandTo18Decimals, getAmountOut, getAmountIn } = require("./utilities");
...
describe("MiniSwap - Uniswap V2 Test Patterns", function () {
  ...
  it("Should revert swap with insufficient output", async function () {
    ...
    // Try to swap more than available (should revert)
    const excessiveAmount = expandTo18Decimals(100);
    await token0.approve(miniswap.target, excessiveAmount);
    
    // This should revert because output would be insufficient
    await expect(miniswap.swap(token0.target, excessiveAmount))
