...
class Logger {
  ...
  static link(label: string, url: string): void {
...
class UpgradeChecker {
  ...
  private v2Contract: Contract;
  
  constructor() {
    this.provider = new JsonRpcProvider(CONFIG.RPC_URL);
    this.proxyContract = new Contract(CONFIG.PROXY_ADDRESS, ABI.PROXY, this.provider);
    this.v2Contract = new Contract(CONFIG.PROXY_ADDRESS, ABI.V2, this.provider);
  }
  ...
  private async checkNetwork(): Promise<void> {
    ...
    try {
      ...
      Logger.data('最新Gas价格', `${ethers.formatUnits(feeData.gasPrice || 0n, 'gwei')} Gwei`);
      Logger.data('基础费用', `${ethers.formatUnits(feeData.maxFeePerGas || 0n, 'gwei')} Gwei`);
    } catch (error: any) {
  ...
  private async checkContractFunctions(): Promise<void> {
    ...
    try {
      ...
      for (const slot of slots) {
        try {
          ...
          const value = ethers.toBigInt(storageValue);
      ...
      try {
        ...
        if (ethers.toBigInt(storageValue).toString() === contractValue.toString()) {
          Logger.success('✅ 存储槽0与合约返回值一致');
        } else {
          Logger.error(`❌ 存储槽0 (${ethers.toBigInt(storageValue)}) 与合约返回值 (${contractValue}) 不一致`);
        }
      } catch (error) {
        // 忽略错误
      }
    ...
    try {
      ...
      if (tx) {
        ...
        if (tx.data && tx.data.length > 10) {
          const iface = new Interface(["function upgrade(address _newImplementation)"]);
          try {
        ...
        try {
          ...
          if (receipt) {
            ...
            Logger.data('Gas价格', ethers.formatUnits(tx.gasPrice || 0n, 'gwei') + ' Gwei');
